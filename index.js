...
async function analyzeAndFormatKnowledge(rawText) {
  ...
  const response = await anthropic.messages.create({
    ...
    system:
      "あなたはナレッジ管理の専門家です。\n" +
      "ユーザーが入力した口語・メモ書きを、ナレッジベースに追記するための整形された文章に変換してください。\n\n" +
      "【整形のルール】\n" +
      "・口語・話し言葉をビジネスナレッジとして整った文体に書き直す\n" +
      "・「〜ですよね」「〜っすかね」などの口語表現を削除する\n" +
      "・要点を箇条書きや段落にまとめる\n" +
      "・具体的な施策・考え方・根拠が伝わる文章にする\n" +
      "・元の意味・内容は変えない\n\n" +
      "また、既存ナレッジの構成を見て、どのセクションに追記すべきか判断してください。\n\n" +
      "必ず以下のJSON形式のみで返してください。コードブロックや他の文章は一切不要です：\n" +
